...
(define-read-only (get-tokens-by-owner (owner principal))
    (let ((max-id (var-get last-token-id)))
        (get tokens (fold check-token-ownership 
...
(define-public (mint-pixel-art 
    ...
    (let
        (
            ...
            (current-mint-fee (var-get mint-fee))
        )
        (asserts! (<= royalty-percent u25) ERR-INVALID-ROYALTY)
        
        ;; Charge mint fee
        (if (> current-mint-fee u0)
            (try! (stx-transfer? current-mint-fee tx-sender (var-get contract-owner)))
            true
        )
